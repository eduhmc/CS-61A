...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect == QueenAnt.true_queen:
                return 
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.opposite_direction  == True:
            destination = self.place.entrance
            if type(destination) == Hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        counter, actual_place = 0 , self.place
        while actual_place.entrance!=None or actual_place!=hive:
            if actual_place.bees and counter >= self.min_range and counter <= self.max_range:
                return random_or_none(actual_place.bees)
            else:
                actual_place = actual_place.entrance
                counter+=1
        return None
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    implemented = True #False   # Change to True to view in the GUI
    food_cost = 2
    min_range =0
    max_range = 3
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        [b.reduce_armor(self.damage) for b in list(self.place.bees)]
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self is not QueenAnt.true_queen:
            ...
            while another_place is not None:
                if another_place.ant != None:
                    if another_place.ant.is_container and another_place.ant.contained_ant:
                        if another_place.ant.contained_ant not in QueenAnt.doubles:
                            another_place.ant.contained_ant.damage *= 2 
                            QueenAnt.doubles.append(another_place.ant.contained_ant)
                        if another_place.ant not in QueenAnt.doubles:
                            another_place.ant.damage *= 2 
                            QueenAnt.doubles.append(another_place.ant)
                

                    elif another_place.ant not in QueenAnt.doubles:
                        another_place.ant.damage *= 2 
                        QueenAnt.doubles.append(another_place.ant)

                another_place = another_place.exit
...
def make_slow(action, bee):
    ...
    def action_method(colony):
        if colony.time % 2 == 0: #when colony.time is even
...
def apply_effect(effect, bee, duration):
    ...
    def current_action(colony):
        nonlocal i 
        if i >= duration: # after duration
            return prev_action(colony)
        else:
            i = i + 1
            return effect(prev_action, bee)(colony)
