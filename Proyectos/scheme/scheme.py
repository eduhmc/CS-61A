...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        procedure = scheme_eval(first, env)
        check_procedure(procedure)
        if isinstance(procedure, MacroProcedure): #changed for EC
...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions.first, env, True) #True for EC
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if isinstance(expressions.first, Pair):
            ...
            expression_evaluated = scheme_eval(expressions.second.first, env)
            env.define(expressions.first, expression_evaluated)
        ...
        procedure = do_lambda_form(Pair(parameters, body), env)
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    #f check_form(expressions.first, quote):
    return expressions.first
    # END PROBLEM 7
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        ...
        return scheme_eval(expressions.second.second.first, env, True) #True for EC 20
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    while expressions.second != nil:
        evaluating = scheme_eval(expressions.first, env)
        if scheme_falsep(evaluating):
            return evaluating
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True) #True for EC 20
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions, 2)
    aim = expressions.first
    if isinstance(aim, Pair):
        if scheme_symbolp(aim.first):
            check_formals(aim.second)
            env.define(aim.first, MacroProcedure(aim.second, expressions.second, env))
            return aim.first
    raise SchemeError("incorrect definition")
    # END Problem 21
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    "*** YOUR CODE HERE ***"
    new = MuProcedure(formals, expressions.second)
    return new
