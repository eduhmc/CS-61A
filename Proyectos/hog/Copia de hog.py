...
def is_swap(player_score, opponent_score):
    ...
    """
    #cleaner way but using thing they havent teach us
    if player_score > 0 and opponent_score > 0:

        player_score = [int(d) for d in str(player_score)]
        opponent_score = [int(d) for d in str(opponent_score)]#turn number into digits in list

        if len(player_score) == 1 and len(opponent_score) == 3:#0.7 is a place holder number
            player_score.insert(0,0.7)
            player_score.insert(1,0.7)
        if len(opponent_score) == 1 and len(player_score) == 3:
            opponent_score.insert(0,0.7)
            opponent_score.insert(1,0.7)

        if len(player_score) < len(opponent_score):
            player_score.insert(0,0.7)
        if len(player_score) > len(opponent_score):
            opponent_score.insert(0,0.7)

        if len(player_score) == len(opponent_score): #check if the same digit appear in the same position
            for number in range(len(player_score)):
                if player_score[number] == opponent_score[number]:
                    return True
            return False
    if player_score == 0 or opponent_score == 0: #if one of the number is zero condition
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 <= goal and score1 <= goal:
        ...
        if player == 0:
            ...
            op_score = score1
            strategy = strategy0
            op_strategy = strategy1
        
        elif player == 1:
            score = score1
            op_score = score0
            strategy = strategy1
            op_strategy = strategy0
        
        num_rolls = strategy(score, op_score)
        add_score = take_turn(num_rolls, op_score, dice)
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """  
    #Solution with my partner
    first = opponent_score // 10
    second = opponent_score % 10
    if (min(first, second)+1) >= margin:
        return 0
    return num_rolls
    """
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """ 
    #Solution with my partner
    after_roll_dice = free_bacon(opponent_score) + score

    if is_swap(after_roll_dice, opponent_score):
        if opponent_score > after_roll_dice:
            return 0

    if not(is_swap(after_roll_dice, opponent_score)):
        if free_bacon(opponent_score) >= margin:
            return 0

    if after_roll_dice == opponent_score and free_bacon(opponent_score)>= margin:
        return 0
